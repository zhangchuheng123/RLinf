...
# Background:
# - The increase_net setting improved capacity but actor update remains conservative.
# Goal:
# - Keep increase_net architecture fixed, increase actor update strength, and use normalized advantages.
# Changes vs 20260331 increase_net:
# - Increase `actor.optim.dsrl_actor_lr` from `1.0e-5` to `1.0e-4`.
# - Enable `algorithm.do_adv_norm=true`, `algorithm.adv_norm_eps=1.0e-8`, `algorithm.advantage_clip=3.0`.
...
CONFIG_NAME="20260401_scalar_mc_env16_exec4_mb2048_increase_actor_lr"
