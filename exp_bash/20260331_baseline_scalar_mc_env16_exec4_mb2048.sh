...
# Background:
# - Current best-known run is libero_10_dsrl_smolvla_scalar_ddp16x4_mc.
# Goal:
# - Reproduce the strongest scalar MC baseline with global algorithm settings fixed across machines.
# Changes vs previous baseline:
# - No algorithm change. This script is self-contained and does not call examples/.
